...
module.exports = function (token, options) {
    ...
    return function (req, res, next) {
        ...
        if (parsedUrl.path && parsedUrl.path.match(/.*(\.[^?]{2,4}$|\.[^?]{2,4}?.*)/))
            return next();
        if (req.headers['user-agent'] && req.headers['user-agent'].match(/(bot|spider|pinterest|crawler|archiver|flipboard|mediapartners|facebookexternalhit|quora|whatsapp)/gi))
            return serveCapture();
        return next();
    };
};
