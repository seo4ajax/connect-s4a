...
module.exports = function (token, options) {
    ...
    return function (req, res, next) {
        ...
        if (parsedUrl.query && (parsedUrl.query['_escaped_fragment_'] != null))
            return serveCapture();
        if ((!options || !options.dynamicRendering) && req.headers['user-agent'] && req.headers['user-agent'].match(/(googlebot\/|googlebot-mobile|yandexbot|pinterest.*ios|mail\.ru|seznambot|screaming)/gi))
            return next();
        if (parsedUrl.path && parsedUrl.path.match(/.*(\.[^?]{2,4}$|\.[^?]{2,4}?.*)/))
            return next();
        if (req.headers['user-agent'] && req.headers['user-agent'].match(/(bot|spider|pinterest|crawler|archiver|flipboardproxy|mediapartners|facebookexternalhit|quora)/gi))
            return serveCapture();
        return next();
    };
};
